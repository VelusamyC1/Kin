-- 50 fake players for dev/test (runs only when db/seed location is active — dev profile)
...
BEGIN
  FOREACH r SLICE 1 IN ARRAY players LOOP
    ...
    VALUES (uid, r[1], pass, r[3], r[4], r[5], r[6], false)
    ON CONFLICT (email) DO NOTHING;

    -- re-fetch id in case of conflict
    SELECT id INTO uid FROM users WHERE email = r[1];
